...
import { ILayersPanelPluginContext } from './tree-master';
...
export class TreeNode {
	...
	get expandable(): boolean {
		...
		return (
			this.hasChildren() || this.hasSlots() || this.dropDetail?.index != null
		);
	}
	...
	get children(): TreeNode[] | null {
		return (
			this.node.children?.map((node) => this.tree.getTreeNode(node)) || null
		);
	}

	get node(): IPublicModelNode {
		return toRaw(this._node);
	}
	...
	isRoot(includeOriginalRoot = false) {
		...
		return (
			this.tree.root === this || (includeOriginalRoot && rootNode === this.node)
		);
	}
