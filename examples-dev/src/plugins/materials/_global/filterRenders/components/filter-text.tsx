import { Alert, Input, InputGroup, RadioGroup, Select } from 'ant-design-vue';
	...
	setup(props, { emit }) {
		...
		return () => (
			<div style="display: flex; flex-direction: column; gap: 12px">
				...
				<InputGroup compact>
					<Select v-model:value={firstQueryCondition.value} style="width: 30%">
				...
				<InputGroup compact>
					<Select v-model:value={secondQueryCondition.value} style="width: 30%">
						{Object.values(ComparisonOperator).map((operator) => (
							<Select.Option key={operator} value={operator}>
								{operator === ComparisonOperator.EMPTY ? '' : operator}
							</Select.Option>
