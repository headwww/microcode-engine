...
import { isFormEvent, isNode, isNodeSchema } from '@arvin-shu/microcode-utils';
...
function insertChild(
	...
	if (isNode(child) && (child as IPublicModelNode).isSlotNode) {
		child = (child as IPublicModelNode).exportSchema(
	...
	if (isNode(child)) {
		...
		node = container.document?.createNode(child as any) || null;
		if (node) {
			container.children?.insert(node, at);
		}
	}
...
function getSuitableInsertion(
	...
	if (
		target &&
		insertNode &&
		!target.componentMeta?.checkNestingDown(target, insertNode)
	) {
		return null;
	}
...
export const builtinHotkey = (ctx: IPublicModelPluginContext) => ({
	init() {
		const { hotkey, canvas, project } = ctx;
		...
		hotkey.bind(['command+c', 'ctrl+c', 'command+x', 'ctrl+x'], (e, action) => {
			...
			selected = selected.filter((node) => node?.canPerformAction('copy'));
		...
		hotkey.bind(['command+v', 'ctrl+v'], (e) => {
			...
			clipboard.waitPasteData(e, ({ componentsTree }) => {
				if (componentsTree) {
					const { target, index } =
						getSuitableInsertion(ctx, componentsTree) || {};
					if (!target) {
						return;
					}
					const canAddComponentsTree = componentsTree.filter(
						(node: IPublicModelNode) => {
							const dragNodeObject: IPublicTypeDragNodeObject = {
								type: IPublicEnumDragObjectType.Node,
								nodes: [node],
		...
		hotkey.bind(['left', 'right'], (e, action) => {
			...
			const silbing =
				action === 'left' ? firstNode?.prevSibling : firstNode?.nextSibling;
			silbing?.select();
		});
