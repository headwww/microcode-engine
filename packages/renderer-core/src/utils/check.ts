...
export function isESModule<T>(obj: T | { default: T }): obj is ESModule {
	return (
		obj &&
		(Reflect.get(obj, '__esModule') ||
			Reflect.get(obj, Symbol.toStringTag) === 'Module')
	);
}
...
export function isJSExpression(val: unknown): val is IPublicTypeJSExpression {
	return (
		isObject(val) && val.type === 'JSExpression' && val.extType !== 'function'
	);
}
