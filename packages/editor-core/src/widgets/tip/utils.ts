...
function resolveEdge(popup: any, target: any, arrow: any, bounds: any) {
	...
	const sy =
		arrow.width > target.height ? (arrow.width - target.height) / 2 : 0;

	const top = Math.max(
		target.top - popup.height + arrow.width - sy,
		bounds.top
	);
	const right = Math.min(
		target.right + popup.width - arrow.width + sx,
		bounds.right
	);
	const bottom = Math.min(
		target.bottom + popup.height - arrow.width + sy,
		bounds.bottom
	);
	const left = Math.max(
		target.left - popup.width + arrow.width - sx,
		bounds.left
	);
...
function resolvePrefer(prefer: any, targetRect: any, bounds: any) {
	if (!prefer) {
		...
		if (targetRect.top - bounds.top < 10) {
			return { dir: 'bottom' };
		}
		if (bounds.bottom - targetRect.bottom < 10) {
			return { dir: 'top' };
		}
		if (bounds.right - targetRect.right < 10) {
			return { dir: 'left' };
		}
		return {};
	}
...
export function resolvePosition(
	...
	if (dir === 'top' || dir === 'bottom') {
		...
		// 解决横向偏移
		const offset =
			arrow.width > target.width ? (arrow.width - target.width) / 2 : 0;
...
export function resolveRelativePosition(align: any, popup: any, bounds: any) {
	...
	return {
		left: resolveRelativeValue(
			xAlign,
			0,
			bounds.right - bounds.left - popup.width
		),
		top: resolveRelativeValue(
			yAlign,
			0,
			bounds.bottom - bounds.top - popup.height
		),
	};
}
