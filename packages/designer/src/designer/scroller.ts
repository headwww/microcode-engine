...
import { isElement } from '@arvin-shu/microcode-utils';
...
export class Scroller implements IScroller {
	...
	scrolling(point: { globalX: number; globalY: number }) {
		...
		if (y < bounds.top + SCROLL_ACCURACY) {
			ay =
				-Math.min(Math.max(bounds.top + SCROLL_ACCURACY - y, 10), 50) / scale;
		} else if (y > bounds.bottom - SCROLL_ACCURACY) {
			ay =
				Math.min(Math.max(y + SCROLL_ACCURACY - bounds.bottom, 10), 50) / scale;
		}
		if (x < bounds.left + SCROLL_ACCURACY) {
			...
			ax =
				Math.min(Math.max(x + SCROLL_ACCURACY - bounds.right, 10), 50) / scale;
		}
