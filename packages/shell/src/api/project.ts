...
export class Project implements IPublicApiProject {
	...
	get documents(): IPublicModelDocumentModel[] {
		return this[projectSymbol].documents.map(
			(doc) => ShellDocumentModel.create(doc)!
		);
	}
	...
	getDocumentByFileName(fileName: string): IPublicModelDocumentModel | null {
		const innerDocumentModel =
			this[projectSymbol].getDocumentByFileName(fileName);
	...
	onSimulatorHostReady(
		...
		const offFn = this[projectSymbol].onSimulatorReady(
