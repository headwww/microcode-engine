...
export class DocumentModel implements IPublicModelDocumentModel {
	 ...
	 */
	onMountNode(
		fn: (payload: { node: IPublicModelNode }) => void
	): IPublicTypeDisposable {
	 ...
	 */
	onChangeDetecting(
		fn: (node: IPublicModelNode) => void
	): IPublicTypeDisposable {
		return this[documentSymbol].designer.detecting.onDetectingChange(
			(node: InnerNode) => {
				fn(ShellNode.create(node)!);
			}
		);
	...
	onChangeNodeVisible(
		...
		return this[documentSymbol].onChangeNodeVisible(
			(node: InnerNode, visible: boolean) => {
