...
import { nodeChildrenSymbol, nodeSymbol } from '../symbols';
...
export class NodeChildren implements IPublicModelNodeChildren {
	...
	splice(start: number, deleteCount: number, node?: IPublicModelNode): any {
		this[nodeChildrenSymbol].splice(
			start,
			deleteCount,
			(node as any)?.[nodeSymbol]
		);
	}
	...
	reverse(): IPublicModelNode[] {
		return this[nodeChildrenSymbol].reverse().map((d) => ShellNode.create(d)!);
	}
	...
	map<T = any>(fn: (node: IPublicModelNode, index: number) => T): T[] | null {
		return this[nodeChildrenSymbol].map<T>(
			(item: InnerNode, index: number): T => fn(ShellNode.create(item)!, index)
		);
	}
	 ...
	 */
	reduce(
		fn: (acc: any, cur: IPublicModelNode) => any,
		initialValue: any
	): void {
		return this[nodeChildrenSymbol].reduce(
			(acc: any, cur: InnerNode) => fn(acc, ShellNode.create(cur)!),
			initialValue
		);
	}
